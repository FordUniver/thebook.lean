...
lemma mersenne_prime_dvd_gt (p q : ℕ) (p_prime: Nat.Prime p) (q_prime: Nat.Prime q) (h : q ∣ 2^p - 1) : p < q := by
  ...
  have two_ne_zero : (2 : ZMod q) ≠ 0 := by
    ...
    have q_gt_two : q > 2 := Nat.lt_of_le_of_ne (Nat.Prime.two_le q_prime) (Odd.ne_two_of_dvd_nat (Nat.Even.sub_odd (Nat.one_le_two_pow) ( even_iff_two_dvd.mpr (dvd_pow_self 2 (Nat.Prime.ne_zero p_prime))) (Exists.intro Nat.zero rfl)) h).symm
