...
lemma F_odd (n : ℕ) : Odd (F n) :=
  Even.add_one ((Nat.even_pow' (NeZero.ne (2 ^ n))).mpr (Nat.even_iff.mpr rfl))
...
lemma coprime_iff_divisors_one (k n : ℕ) : Nat.Coprime k n ↔ ∀ m, m ∣ k ∧ m ∣ n → m = 1 :=
  ⟨λ k_n_coprime _ ↦
    (λ ⟨m_dvd_k, m_dvd_n⟩ ↦ Nat.eq_one_of_dvd_coprimes k_n_coprime m_dvd_k m_dvd_n),
...
lemma F_coprime (k n : ℕ) (k_ne_n: k ≠ n) : Nat.Coprime (F k) (F n) := by
  ...
  · exact this
      n k k_ne_n.symm
      m m_dvd_Fn m_dvd_Fk
      (lt_of_le_of_ne (le_of_not_gt k_lt_n) k_ne_n.symm)
  · have m_dvd_two  : m ∣ 2 := by
      rw [←Nat.sub_sub_self (F_ge_two n), ←F_prod_form]
      exact Nat.dvd_sub'
        m_dvd_Fn
        (Nat.dvd_trans m_dvd_Fk (dvd_prod_of_mem F (mem_range.mpr k_lt_n)))
    ...
    have m_ne_two   : m ≠ 2    := λ m_eq_two ↦
      Nat.not_odd_iff_even.mpr (even_iff_two_dvd.mpr (m_eq_two ▸ m_dvd_Fn)) (F_odd n)
    exact Nat.eq_of_lt_succ_of_not_lt
      (lt_of_le_of_ne m_le_two m_ne_two)
      (Nat.not_lt.mpr (Nat.one_le_iff_ne_zero.mpr m_ne_zero))
...
theorem infinitude_of_primes : ∃ P : ℕ → ℕ, Injective P ∧ ∀ k, (P k).Prime := by
  ...
  have P_inj : Injective P := by
    ...
    have Pm_dvd_gcd   : P m ∣ Nat.gcd (F m) (F n) :=
      Nat.dvd_gcd (P_dvd_fermat m) (Pm_eq_Pn ▸ P_dvd_fermat n)
    by_contra m_ne_n
