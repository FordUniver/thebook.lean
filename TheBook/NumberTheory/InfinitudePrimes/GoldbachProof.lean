...
lemma test : (2 ^ 2 ^ n - 1) * (2 ^ 2 ^ n + 1) = 2 ^ (2 ^ Nat.succ n) - 1 := by
  simp [Nat.mul_sub_right_distrib, Nat.mul_add]
  rw [tsub_add_tsub_cancel (Nat.le_mul_self (2 ^ 2 ^ n)) (Nat.one_le_two_pow (2 ^ n))]
...
lemma F_coprime (k n : ℕ) (k_ne_n: k ≠ n) : Nat.Coprime (F k) (F n) := by
  ...
  · have m_dvd_two : m ∣ 2 := by
      have two_through_fermat : 2 = (F n - (∏ k in range n, F k)) := by
        rw [F_prod_form, Nat.sub_sub_self (F_ge_two n)]
