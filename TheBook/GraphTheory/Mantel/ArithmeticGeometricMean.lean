...
theorem mantel (h: G.CliqueFree 3) : #E ≤ n^2 / 4 := by
  ...
  have hA : #A = α := maximumIndependentSet_card_eq_cocliqueNum G A maxA

  -- ... and set `β = n - α`.
  let β := n - α
  have hαβ: α + β = n := Nat.add_sub_of_le (le_of_eq_of_le (hA.symm) (card_le_card (subset_univ A)))
  ...
  have degree_leq_cardA : ∀ (i : γ) , d(i) ≤ α := fun i => 
  ...
  have one_geq_n_adj_verts : ∀ e ∈ E, 1 ≤ #{ i ∈ B | i ∈ e } := by
  ...
  have count_edges_by_B := by calc
    #E = ∑ e ∈ E, 1                          := by simp
     _ ≤ ∑ e ∈ E, #{ i ∈ B | i ∈ e }         := sum_le_sum one_geq_n_adj_verts
  ...
  have four_times_card_E_bd := calc 
    4 * #E ≤ 4 * α * β        := by linarith
         _ ≤ (α + β)^2        := four_mul_le_pow_two_add α β
