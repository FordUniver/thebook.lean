...
#check Finset.sum_card_bipartiteAbove_eq_sum_card_bipartiteBelow -- probably the double counting equality we want
...
theorem mantel (h: G.CliqueFree 3) : G.edgeFinset.card ≤ (Fintype.card V)^2 / 4 := by
  ...
  have : (∑ (v : V), G.degree v)^2 / n = 4 * E.card^2 / n := by sorry -- SimpleGraph.sum_degrees_eq_twice_card_edges
  sorry
